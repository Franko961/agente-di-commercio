...
class BonusTier(BaseModel):
    threshold: float = Field(
        ge=0, le=MAX_MONETARY_TARGET
    )  # fatturato minimo per ottenere il bonus
...
class MandanteIn(BaseModel):
    ...
    commission_rate_new: Optional[float] = Field(
        None, ge=0, le=100
    )  # override su vendite nuove, se impostato
    ...
    bonus_tiers: Optional[List[BonusTier]] = Field(
        default_factory=list, max_length=MAX_BONUS_TIERS
    )
