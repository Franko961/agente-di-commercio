...
from models.leave_request import LeaveRequestAdminIn, LeaveRequestIn
...
class FakeLeaveRequestRepo:
    ...
    async def find_overlapping(self, user_id, date_from, date_to, status="approvata"):
        return [
            ...
            and d["status"] == status
            and d["date_from"] <= date_to
            and d["date_to"] >= date_from
        ]
...
def build_leave_service(monkeypatch, employees_repo, manager=USER):
    ...
    monkeypatch.setattr(
        leave_request_mod, "check_and_record", lambda *a, **kw: _allow()
    )
...
def make_employee(name="Mario Rossi", **overrides):
    payload = EmployeeIn(
        name=name, role=overrides.get("role", ""), email=overrides.get("email")
    )
    return payload
...
def test_get_by_token_rifiuta_se_modulo_personale_disattivato():
    ...
    owner_senza_modulo = {
        "id": "user-9",
        "email": "senza-modulo@example.com",
...
def test_get_manifest_for_token_punta_esattamente_a_questo_link():
    ...
    manifest = run(
        service.get_manifest_for_token(employee["request_token"], "https://salesfly.it")
    )
...
def test_get_manifest_for_token_rifiuta_dipendente_disattivato():
    ...
    with pytest.raises(NotFoundError):
        run(
            service.get_manifest_for_token(
                employee["request_token"], "https://salesfly.it"
            )
        )
...
def test_submit_rifiuta_se_modulo_personale_disattivato(monkeypatch):
    ...
    service, repo = build_leave_service(
        monkeypatch, emp_repo, manager=owner_senza_modulo
    )
    payload = LeaveRequestIn(
        employee_token=employee["request_token"],
        type="ferie",
        date_from="2026-08-01",
        date_to="2026-08-05",
    )
...
def test_submit_rispetta_il_rate_limit_per_token(monkeypatch):
    ...
    service = LeaveRequestService(
        repo=repo, employees=emp_repo, users=FakeUserRepo({USER["id"]: USER})
    )
...
def test_list_requests_non_segnala_richieste_di_dipendenti_diversi(monkeypatch):
    ...
    run(
        service.submit(
            LeaveRequestIn(
                employee_token=e1["request_token"],
                type="ferie",
                date_from="2026-08-10",
                date_to="2026-08-15",
            )
    ...
    run(
        service.submit(
            LeaveRequestIn(
                employee_token=e2["request_token"],
                type="ferie",
                date_from="2026-08-10",
                date_to="2026-08-15",
            )
...
def test_submit_registra_ma_non_fallisce_se_email_al_responsabile_fallisce(
    monkeypatch, caplog
):
...
def test_decide_registra_ma_non_fallisce_se_email_al_dipendente_fallisce(
    monkeypatch, caplog
):
    emp_service, emp_repo = build_employee_service()
    employee = run(
        emp_service.create_employee(USER, make_employee(email="dipendente@example.com"))
    )
    service, repo = build_leave_service(monkeypatch, emp_repo)
    payload = LeaveRequestIn(
        employee_token=employee["request_token"],
...
def _freeze_today(monkeypatch, year, month, day):
    ...
    monkeypatch.setattr(
        leave_request_mod, "now_local", lambda: real_datetime(year, month, day, 10, 0)
    )
...
def test_employee_summary_calcola_ferie_godute_e_residue(monkeypatch):
    ...
    emp_service, emp_repo = build_employee_service()
    employee = run(
        emp_service.create_employee(USER, make_employee())
    )  # annual_vacation_days default 26
    service, repo = build_leave_service(monkeypatch, emp_repo)
...
def test_employee_summary_ignora_richieste_non_approvate_per_le_ferie_godute(
    monkeypatch,
):
    _freeze_today(monkeypatch, 2026, 8, 20)
    emp_service, emp_repo = build_employee_service()
...
def test_employee_summary_malattia_a_cavallo_capodanno_conta_solo_i_giorni_dell_anno(
    monkeypatch,
):
    ...
    summary = run(service.employee_summary(USER, employee))
    assert (
        summary["malattie"]["giorni"] == 5
    )  # solo i giorni del 2026 (1,2,3,4,5 gennaio)
    assert len(summary["malattie"]["richieste"]) == 1
    richiesta = summary["malattie"]["richieste"][0]
    assert richiesta["giorni"] == 5
    assert (
        richiesta["date_from"] == "2025-12-29"
    )  # l'intervallo reale resta quello originale
    assert richiesta["date_to"] == "2026-01-05"


def test_employee_summary_malattia_a_cavallo_capodanno_conta_i_giorni_dell_anno_precedente(
    monkeypatch,
):
...
def test_employee_summary_ferie_a_cavallo_capodanno_conta_solo_i_giorni_dell_anno(
    monkeypatch,
):
    _freeze_today(monkeypatch, 2026, 1, 15)
    emp_service, emp_repo = build_employee_service()
...
def test_employee_summary_ferie_conteggio_festivita_esclude_domenica_e_festivita_ma_include_sabato(
    monkeypatch,
):
...
def test_create_by_admin_registra_direttamente_approvata_senza_email(monkeypatch):
    ...
    payload = LeaveRequestAdminIn(
        type="smartworking", date_from="2026-08-05", date_to="2026-08-05"
    )
...
def test_create_by_admin_rifiuta_dipendente_di_un_altro_utente(monkeypatch):
    ...
    payload = LeaveRequestAdminIn(
        type="trasferta", date_from="2026-08-05", date_to="2026-08-05"
    )
    with pytest.raises(NotFoundError):
...
def test_create_by_admin_rifiuta_dipendente_inesistente(monkeypatch):
    ...
    payload = LeaveRequestAdminIn(
        type="reperibilita", date_from="2026-08-05", date_to="2026-08-05"
    )
    with pytest.raises(NotFoundError):
...
def test_create_by_admin_rifiuta_intervallo_invertito(monkeypatch):
    ...
    payload = LeaveRequestAdminIn(
        type="straordinari", date_from="2026-08-05", date_to="2026-08-01", hours=1
    )
    with pytest.raises(ValidationAppError):
        run(service.create_by_admin(USER, employee["id"], payload))
