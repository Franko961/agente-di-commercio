...
from tests.test_subscription_cancel_grace_period import FakeRequest, FakeUserRepo
    ...
    with pytest.raises(HTTPException) as exc_info:
        run(
            service.handle_stripe_webhook(
                FakeRequest(headers={"stripe-signature": "qualunque"})
            )
        )
...
def test_webhook_stripe_funziona_normalmente_con_webhook_secret_configurato(
    monkeypatch,
):
