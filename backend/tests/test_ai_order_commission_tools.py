...
def test_add_order_richiede_sempre_conferma():
    ...
    assert (
        service.requires_confirmation("add_order", {"total_amount": 1}, "voice") is True
    )
...
def test_add_commission_richiede_sempre_conferma():
    ...
    assert (
        service.requires_confirmation("add_commission", {"amount": 1}, "voice") is True
    )
...
def test_prepare_add_order_cliente_non_trovato():
    ...
    result = run(
        service.prepare_add_order(
            {
                "client_name": "Cliente Inesistente",
                "mandante_name": "Paginesi",
                "total_amount": 500,
            },
            FAKE_USER["id"],
...
def test_prepare_add_order_mandante_non_trovato():
    ...
    result = run(
        service.prepare_add_order(
            {
                "client_name": "Rossi",
                "mandante_name": "Mandante Inesistente",
                "total_amount": 500,
            },
            FAKE_USER["id"],
        )
...
def test_prepare_add_order_importo_zero_e_errore():
    ...
    result = run(
        service.prepare_add_order(
            {"client_name": "Rossi", "mandante_name": "Paginesi", "total_amount": 0},
            FAKE_USER["id"],
        )
    )
...
def test_finalize_add_order_happy_path_delega_a_order_service():
    ...
    with patch(
        "services.ai_service.actions.orders.order_service"
    ) as mock_order_service:
        mock_order_service.create_order = AsyncMock(
            return_value={"id": "o-1", "numero_ordine": "ORD-0001", "total": 1000}
        )
        msg = run(
            service._finalize_add_order(
                FAKE_USER["id"],
                {
                    "client_id": "c-1",
...
def test_finalize_add_order_annullato_non_genera_provvigione():
    ...
    with patch(
        ...
        mock_order_service.create_order = AsyncMock(
            return_value={"id": "o-1", "numero_ordine": "ORD-0002", "total": 500}
        )
        msg = run(
            service._finalize_add_order(
                FAKE_USER["id"],
...
def test_finalize_add_order_importo_modificato_ricalcola_items():
    ...
    with patch(
        ...
        mock_order_service.create_order = AsyncMock(
            return_value={"id": "o-1", "numero_ordine": "ORD-0003", "total": 2000}
        )
        run(
            service._finalize_add_order(
                FAKE_USER["id"],
...
def test_prepare_add_commission_happy_path_con_mandante_e_cliente():
    ...
    result = run(
        service.prepare_add_commission(
            {
                "amount": 300,
                "period": "2026-05",
                "mandante_name": "Paginesi",
                ...
                "tipo": "bonus",
                "stato": "incassato",
                "descrizione": "Premio trimestrale",
            },
            FAKE_USER["id"],
        )
    )
...
def test_prepare_add_commission_mandante_e_cliente_non_trovati_non_blocca():
    ...
    result = run(
        service.prepare_add_commission(
            {
                "amount": 150,
                "mandante_name": "Mandante Fantasma",
...
def test_prepare_add_commission_periodo_non_valido_e_errore():
    ...
    result = run(
        service.prepare_add_commission(
            {"amount": 100, "period": "non-un-mese"}, FAKE_USER["id"]
        )
    )
...
def test_finalize_add_commission_happy_path_delega_a_commission_service():
    ...
    with patch(
        "services.ai_service.actions.commissions.commission_service"
    ) as mock_commission_service:
        mock_commission_service.create_manual_commission = AsyncMock(
            return_value={"id": "mc-1"}
        )
        msg = run(
            service._finalize_add_commission(
                FAKE_USER["id"],
                {
                    "period": "2026-05",
                    "amount": 300,
    ...
    called_user, fields = (
        mock_commission_service.create_manual_commission.await_args.args
    )
    assert called_user == {"id": FAKE_USER["id"]}
        ...
        msg = run(
            service._finalize_add_commission(
                FAKE_USER["id"],
                {
                    ...
                    "mandante_id": None,
                    "mandante_name": None,
                    "mandante_not_found": "Mandante Fantasma",
                    "client_id": None,
                    "client_name": None,
                },
            )
...
def test_add_order_non_scrive_subito_ma_richiede_conferma():
    responses = {
        "responses": [
            ...
            make_message(
                [
                    make_text_block(
                        "Ho preparato l'ordine per Rossi Srl da 900€, conferma per registrarlo."
                    )
                ],
    ...
    result = run(
        service.chat(
            FAKE_USER, Payload("registra un ordine di 900 euro per Rossi con Paginesi")
        )
    )
...
def test_add_commission_non_scrive_subito_ma_richiede_conferma():
    responses = {
        "responses": [
            make_message(
                [
                    make_tool_use_block(
                        "add_commission", {"amount": 250, "tipo": "bonus"}, "tu_1"
                    )
                ],
                stop_reason="tool_use",
            ),
            make_message(
                [
                    make_text_block(
                        "Ho preparato la provvigione manuale da 250€, conferma per registrarla."
                    )
                ],
                stop_reason="end_turn",
            ),
    ...
    result = run(
        service.chat(
            FAKE_USER,
            Payload("registra una provvigione manuale di 250 euro come bonus"),
        )
    )
...
def _prepare_pending_order(service):
    return run(
        service._log_action(
            FAKE_USER["id"],
            "chat",
            "registra ordine 900 euro Rossi Paginesi",
            "add_order",
...
def test_execute_confirmed_action_add_order_ignora_client_id_manomesso():
    ...
    with patch(
        ...
        mock_order_service.create_order = AsyncMock(
            return_value={"id": "o-1", "numero_ordine": "ORD-1", "total": 900}
        )
        run(
...
def test_execute_confirmed_action_add_order_amount_e_status_restano_modificabili():
    ...
    with patch(
        ...
        mock_order_service.create_order = AsyncMock(
            return_value={"id": "o-1", "numero_ordine": "ORD-1", "total": 1800}
        )
        result = run(
            service.execute_confirmed_action(
                FAKE_USER,
...
def _prepare_pending_commission(service):
    return run(
        service._log_action(
            FAKE_USER["id"],
            "chat",
            "registra provvigione manuale 300 euro",
            "add_commission",
            {"amount": 300},
            status="in_attesa",
            resolved_params={
...
def test_add_order_bloccato_se_modulo_ordini_disattivato():
    ...
    result = run(
        service.chat(
            user, Payload("registra un ordine di 500 euro per Rossi con Paginesi")
        )
    )
...
def test_add_commission_bloccato_se_modulo_provvigioni_disattivato():
    responses = {
        "responses": [
            make_message(
                [make_tool_use_block("add_commission", {"amount": 300}, "t1")],
                stop_reason="tool_use",
            ),
            make_message([make_text_block("Fatto.")], stop_reason="end_turn"),
        ]
    ...
    result = run(
        service.chat(user, Payload("registra una provvigione manuale di 300 euro"))
    )

    assert result["pending_actions"] == []
    log = next(
        d for d in service.action_log_repo.docs if d["tool_name"] == "add_commission"
    )
    assert log["status"] == "fallita"
    assert "🔒" in log["result"]
