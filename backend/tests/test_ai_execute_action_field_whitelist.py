...
def _prepare_pending_offer(service):
    ...
    return run(
        service._log_action(
            FAKE_USER["id"],
            "chat",
            "registra vendita 1500 euro Rossi Paginesi",
            "add_offer",
...
def test_prodotti_manomessi_dal_browser_vengono_ignorati():
    ...
    tampered["items"] = [
        {
            "product_id": "p-costoso",
            "description": "Prodotto sostituito",
            "quantity": 1000,
            "unit_price": 99999,
            "discount": 0,
        }
...
def test_expense_categoria_e_descrizione_modificabili_ma_non_altri_campi():
    ...
    tampered["client_id"] = (
        "iniettato"  # non previsto per add_expense, deve essere ignorato comunque
    )
