...
def build_service():
    ...
    service = EmployeeCompensationService(
        repo=comp_repo, employees=emp_repo, expenses=expense_repo
    )
...
def test_create_compensation_genera_spesa_collegata():
    ...
    comp = run(
        service.create_compensation(
            USER,
            "emp-1",
            EmployeeCompensationIn(
                type="stipendio",
                amount=1500,
                date=date(2026, 1, 31),
                notes="gennaio",
    ...
    with pytest.raises(ValidationAppError):
        run(
            service.create_compensation(
                USER,
                "emp-does-not-exist",
                EmployeeCompensationIn(amount=100, date=date(2026, 1, 1)),
            )
        )
...
def test_list_compensations_scoped_to_employee_and_user():
    ...
    run(
        service.create_compensation(
            USER, "emp-1", EmployeeCompensationIn(amount=1000, date=date(2026, 1, 1))
        )
    )
    run(
        service.create_compensation(
            USER, "emp-2", EmployeeCompensationIn(amount=2000, date=date(2026, 1, 1))
        )
    )
...
def test_update_compensation_sincronizza_la_spesa_collegata():
    ...
    comp = run(
        service.create_compensation(
            USER,
            "emp-1",
            EmployeeCompensationIn(type="bonus", amount=200, date=date(2026, 2, 1)),
        )
    )

    run(
        service.update_compensation(
            USER,
            "emp-1",
            comp["id"],
            EmployeeCompensationIn(
                type="bonus", amount=350, date=date(2026, 2, 15), notes="produttività"
            ),
        )
    )
...
def test_update_compensation_other_user_raises_404():
    ...
    comp = run(
        service.create_compensation(
            USER, "emp-1", EmployeeCompensationIn(amount=100, date=date(2026, 1, 1))
        )
    )
    with pytest.raises(NotFoundError):
        run(
    ...
    with pytest.raises(NotFoundError):
        run(
            service.update_compensation(
                ...
                "emp-2",
                comp["id"],
                EmployeeCompensationIn(amount=999, date=date(2026, 1, 1)),
            )
        )
