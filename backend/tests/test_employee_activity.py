...
def test_get_activity_include_documento_caricato():
    ...
    assert any(
        e["type"] == "documento_caricato" and "Contratto 2026" in e["detail"]
        for e in events
    )
...
def test_get_activity_include_compenso_registrato():
    ...
    assert any(
        e["type"] == "compenso_registrato" and e["detail"] == "Bonus" for e in events
    )
...
def test_get_activity_include_contestazione_registrata():
    ...
    assert any(
        e["type"] == "contestazione_registrata" and "Ritardo ripetuto" in e["detail"]
        for e in events
    )
