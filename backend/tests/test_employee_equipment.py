...
def test_create_equipment_happy_path():
    ...
    item = run(
        service.create_equipment(
            ...
            "emp-1",
            EmployeeEquipmentIn(
                name="Divisa taglia L",
                delivered_date=date(2026, 1, 10),
                notes="consegnata in ufficio",
            ),
        )
    )
...
def test_create_equipment_rejects_unknown_employee():
    ...
    with pytest.raises(ValidationAppError):
        run(
            service.create_equipment(
                USER, "emp-does-not-exist", EmployeeEquipmentIn(name="Chiavi")
            )
        )
...
def test_create_equipment_rejects_other_users_employee():
    ...
    with pytest.raises(ValidationAppError):
        run(
            service.create_equipment(
                OTHER_USER, "emp-1", EmployeeEquipmentIn(name="Chiavi")
            )
        )
...
def test_create_equipment_strips_name_and_notes():
    ...
    item = run(
        service.create_equipment(
            USER, "emp-1", EmployeeEquipmentIn(name="  Casco DPI  ", notes="  nota  ")
        )
    )
...
def test_create_equipment_imposta_updated_at_uguale_a_created_at():
    ...
    service, eq_repo, _ = build_service()
    item = run(
        service.create_equipment(USER, "emp-1", EmployeeEquipmentIn(name="Chiavi"))
    )
...
def test_list_equipment_scoped_to_employee_and_user():
    ...
    run(
        service.create_equipment(
            USER, "emp-1", EmployeeEquipmentIn(name="Telefono aziendale")
        )
    )
    run(
        service.create_equipment(
            USER, "emp-2", EmployeeEquipmentIn(name="Furgone kit primo soccorso")
        )
    )
...
def test_update_equipment_marks_returned():
    ...
    item = run(
        service.create_equipment(
            USER,
            "emp-1",
            EmployeeEquipmentIn(name="Chiavi ufficio", delivered_date=date(2026, 1, 1)),
        )
    )
...
def test_update_equipment_unknown_raises_404():
    ...
    with pytest.raises(NotFoundError):
        run(
            service.update_equipment(
                USER, "emp-1", "does-not-exist", EmployeeEquipmentIn(name="x")
            )
        )
...
def test_update_equipment_other_user_raises_404():
    ...
    with pytest.raises(NotFoundError):
        run(
            service.update_equipment(
                OTHER_USER, "emp-1", item["id"], EmployeeEquipmentIn(name="Chiavi")
            )
        )
...
def test_update_equipment_wrong_employee_id_raises_404():
    ...
    item = run(
        service.create_equipment(
            USER, "emp-1", EmployeeEquipmentIn(name="Chiavi ufficio")
        )
    )
    ...
    with pytest.raises(NotFoundError):
        run(
            service.update_equipment(
                USER, "emp-2", item["id"], EmployeeEquipmentIn(name="Manomesso")
            )
        )
