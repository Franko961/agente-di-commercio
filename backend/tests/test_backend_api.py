...
class TestClients:
    ...
    def test_crud_client(self, client):
        ...
        assert (
            "offers" in d
            and "appointments" in d
            and "documents" in d
            and "commissions" in d
        )
...
class TestAppointments:
    ...
    def test_appt_crud(self, client):
        ...
        aid = r.json()["id"]
        r = client.put(
            f"{API}/appointments/{aid}", json={**payload, "title": "TEST_Updated"}
        )
        assert r.status_code == 200
...
class TestMandanti:
    ...
    def test_mandante_crud(self, client):
        r = client.post(
            f"{API}/mandanti", json={"name": "TEST_Mand", "commission_rate": 7.5}
        )
        assert r.status_code == 200
        mid = r.json()["id"]
        r = client.put(
            f"{API}/mandanti/{mid}",
            json={"name": "TEST_Mand Up", "commission_rate": 8.0},
        )
        assert r.status_code == 200
...
class TestOffers:
    ...
    def test_offer_status_creates_commission(self, client):
        ...
        payload = {
            ...
            "status": "bozza",
            "items": [
                {"description": "Item", "quantity": 2, "unit_price": 100, "discount": 0}
            ],
        }
...
class TestDocuments:
    def test_doc_crud(self, client):
        ...
        assert r.status_code == 200
        r = client.post(
            f"{API}/documents", json={"name": "TEST_Doc", "category": "altro"}
        )
        assert r.status_code == 200
        did = r.json()["id"]
