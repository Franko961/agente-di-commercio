...
class TestUploadSuccess:
    def test_upload_returns_doc_with_required_fields(self, created_doc):
        ...
        assert (
            isinstance(created_doc["storage_path"], str) and created_doc["storage_path"]
        )
...
class TestDownload:
    def test_download_with_bearer_returns_bytes(self, auth_headers, created_doc):
        r = requests.get(
            f"{API}/documents/{created_doc['id']}/download",
            headers=auth_headers,
            timeout=60,
        )
    ...
    def test_download_with_signed_url_token(self, auth_headers, created_doc):
        ...
        r = requests.get(
            f"{API}/documents/{created_doc['id']}/signed-url",
            headers=auth_headers,
            timeout=30,
        )
        assert r.status_code == 200, r.text
    ...
    def test_signed_url_token_non_valido_per_un_altro_documento(
        self, auth_headers, created_doc
    ):
        ...
        try:
            ...
            r = requests.get(
                f"{API}/documents/{other_doc['id']}/download?token={token}", timeout=30
            )
            assert r.status_code == 401
        finally:
    ...
    def test_download_not_found(self, auth_headers):
        r = requests.get(
            f"{API}/documents/nonexistent-id-xyz/download",
            headers=auth_headers,
            timeout=30,
        )
...
class TestSoftDelete:
    def test_soft_delete_removes_from_list(self, auth_headers):
        ...
        r = requests.get(
            f"{API}/documents/{did}/download", headers=auth_headers, timeout=30
        )
        assert r.status_code == 404
