...
router = APIRouter(
    ...
    tags=["route-planning"],
    dependencies=[Depends(require_module("mappa"))],
)
...
async def optimize_route(payload: RoutePlanIn, user=Depends(get_current_user)):
    ...
    allowed = await check_and_record(
        "route_optimize", user["id"], max_attempts=20, window_minutes=10
    )
    if not allowed:
        raise HTTPException(
            429,
            "Troppe richieste di pianificazione giro in poco tempo, riprova tra qualche minuto",
        )
    return await route_optimization_service.plan_day(
        user,
        payload.client_ids,
        payload.start_time,
        payload.visit_minutes,
        payload.start_mode,
        payload.start_lat,
        payload.start_lng,
        payload.round_trip,
