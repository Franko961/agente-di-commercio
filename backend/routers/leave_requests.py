...
admin_router = APIRouter(
    prefix="/api/employees/{eid}/leave-requests", tags=["leave-requests"]
)
...
async def decide_leave_request(
    rid: str, payload: LeaveRequestDecision, user=Depends(forbid_demo_write)
):
...
async def set_leave_request_certificate(
    rid: str, payload: LeaveRequestCertificate, user=Depends(forbid_demo_write)
):
    await leave_request_service.set_certificate_received(
        user, rid, payload.certificate_received
    )
    return {"ok": True}
...
async def create_leave_request_by_admin(
    eid: str, payload: LeaveRequestAdminIn, user=Depends(forbid_demo_write)
):
