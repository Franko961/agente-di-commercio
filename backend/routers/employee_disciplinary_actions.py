...
from services.employee_disciplinary_action_service import (
    employee_disciplinary_action_service,
)

router = APIRouter(
    prefix="/api/employees/{eid}/disciplinary-actions",
    tags=["employee-disciplinary-actions"],
)
...
async def create_employee_disciplinary_action(
    eid: str, payload: EmployeeDisciplinaryActionIn, user=Depends(forbid_demo_write)
):
    return await employee_disciplinary_action_service.create_action(user, eid, payload)


@router.put("/{aid}", dependencies=[MODULE_DEP])
async def update_employee_disciplinary_action(
    eid: str,
    aid: str,
    payload: EmployeeDisciplinaryActionIn,
...
async def delete_employee_disciplinary_action(
    eid: str, aid: str, user=Depends(forbid_demo_write)
):
