...
class HealthService:
    ...
    async def get_health(self, hours: float = 24) -> dict:
            ...
            [
                ...
                },
                {
                    "$project": {
                        ...
                        "count": 1,
                        "avg_duration_ms": {
                            "$round": [{"$divide": ["$sum_duration_ms", "$count"]}, 1]
                        },
        ...
        most_errors = sorted(
            endpoints, key=lambda e: e["status_5xx"] + e["status_4xx"], reverse=True
        )[:10]
        most_errors = [e for e in most_errors if e["status_4xx"] + e["status_5xx"] > 0]

        ai_stats = await self._category_stats(
            "ai_call", since, extra_sum_fields=["cost_usd", "tokens_in", "tokens_out"]
        )
    ...
    async def _category_stats(
        self, category: str, since, extra_sum_fields=None
    ) -> dict:
        group_stage = {
            "_id": "$status",
            "count": {"$sum": 1},
