...
class ClientService:
    ...
    async def get_client(self, user: dict, cid: str) -> dict:
        ...
        offers = await db.offers.find(
            {"client_id": cid, "user_id": user["id"]}, {"_id": 0}
        ).to_list(500)
        ...
        manual_raw = await db.manual_commissions.find(
            {"client_id": cid, "user_id": user["id"]}, {"_id": 0}
        ).to_list(500)
        manual_commissions = [
            normalize_manual_commission(user["id"], m) for m in manual_raw
        ]
        commissions = [
            {**cm, "source": cm.get("source", "order")} for cm in commissions
        ] + manual_commissions
        return {
            "client": c,
            "offers": offers,
    ...
    async def bulk_import(self, user: dict, payload) -> dict:
        ...
        existing_vat = {
            c["vat_number"].strip().upper()
            for c in existing
            if (c.get("vat_number") or "").strip()
        }
        existing_name_city = {
            (
                c.get("company_name", "").strip().lower(),
                ...
                skipped.append(
                    {
                        ...
                        "reason": "ragione sociale mancante",
                    }
                )
                continue
                ...
                skipped.append(
                    {
                        "row": idx,
                        "company_name": item.company_name,
                        "reason": "ragione sociale e città già presenti",
                    }
                )
                continue
