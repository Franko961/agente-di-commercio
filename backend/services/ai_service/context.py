...
from core.utils import local_month_str, now_local
...
async def gather_context(
    ...
    summary = []
    summary.append(
        f"Numero clienti: {len(clients)}, offerte: {len(offers)}, appuntamenti: {len(appts)}"
    )
    summary.append("\nClienti (max 20):")
    for c in clients[:20]:
        ...
        summary.append(
            f"- {c['company_name']} ({c.get('zone', '')}, potenziale {c.get('potential', 'medio')}) ultima visita: {days_ago}gg"
        )
    summary.append("\nOfferte recenti:")
    for o in offers[-10:]:
        summary.append(
            f"- {o.get('title')} importo {o.get('total', 0)}€ stato {o.get('status')}"
        )
    ...
    for c in commissions[-10:]:
        summary.append(
            ...
            + (
                f" (bonus scaglione {c['bonus_tier_threshold']}€)"
                if c.get("sale_type") == "bonus"
                else ""
            )
        )
    ...
    month_expenses = [
        e for e in expenses if (e.get("date") or "").startswith(current_month)
    ]
    ...
    for e in expenses[-10:]:
        summary.append(
            f"- {e.get('date')} {e.get('category')} {e.get('amount', 0)}€ {e.get('description', '')}".strip()
        )
