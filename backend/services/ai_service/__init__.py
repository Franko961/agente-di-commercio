...
from services.ai_service.action_log import list_actions as _list_actions
from services.ai_service.action_log import list_pending_actions as _list_pending_actions
from services.ai_service.action_log import log_action as _log_action
from services.ai_service.action_log import (
    reclaim_stuck_executions as _reclaim_stuck_executions,
)
from services.ai_service.actions._shared import (
    resolve_line_items as _resolve_line_items,
)
from services.ai_service.actions.commissions import (
    finalize_add_commission as _finalize_add_commission,
)
from services.ai_service.actions.commissions import (
    prepare_add_commission as _prepare_add_commission,
)
from services.ai_service.actions.expenses import finalize_expense as _finalize_expense
from services.ai_service.actions.expenses import (
    prepare_add_expense as _prepare_add_expense,
)
from services.ai_service.actions.offers import finalize_offer as _finalize_offer
from services.ai_service.actions.offers import prepare_add_offer as _prepare_add_offer
from services.ai_service.actions.orders import finalize_add_order as _finalize_add_order
from services.ai_service.actions.orders import prepare_add_order as _prepare_add_order
...
from services.ai_service.quota import (
    enforce_ai_message_quota as _enforce_ai_message_quota,
)
from services.ai_service.tools.crm_writer import execute_crm_tool as _execute_crm_tool
from services.ai_service.tools.search import search_clients as _search_clients
from services.ai_service.tools.search import search_offers as _search_offers
...
class AiService:
    ...
    async def list_actions(
        ...
        return await _list_actions(
            self.action_log_repo, user_id, tool_name, status, date_from, date_to, limit
        )
    ...
    async def _resolve_line_items(
        ...
        return await _resolve_line_items(
            self.product_repo, tool_input, user_id, mandante_id, fallback_description
        )

    async def prepare_add_offer(self, tool_input: dict, user_id: str) -> dict:
        return await _prepare_add_offer(
            self.client_repo, self.mandante_repo, self.product_repo, tool_input, user_id
        )

    async def _finalize_offer(self, user_id: str, resolved: dict) -> str:
        return await _finalize_offer(
            self.offer_repo, self.mandante_repo, user_id, resolved
        )

    async def prepare_add_order(self, tool_input: dict, user_id: str) -> dict:
        return await _prepare_add_order(
            self.client_repo, self.mandante_repo, self.product_repo, tool_input, user_id
        )
    ...
    async def prepare_add_commission(self, tool_input: dict, user_id: str) -> dict:
        return await _prepare_add_commission(
            self.mandante_repo, self.client_repo, tool_input, user_id
        )
    ...
    def requires_confirmation(
        self, tool_name: str, tool_input: dict, channel: str = "chat"
    ) -> bool:
