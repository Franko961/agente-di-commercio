...
class AttendanceRepository:
    ...
    async def find_open_session(self, employee_id: str, user_id: str) -> Optional[dict]:
        ...
        return await self.collection.find_one(
            {"employee_id": employee_id, "user_id": user_id, "clock_out": None},
            {"_id": 0},
        )

    async def find_clocked_in_between(
        ...
        return await self.collection.distinct(
            "employee_id",
            {"user_id": user_id, "clock_in": {"$gte": start_iso, "$lt": end_iso}},
        )
    ...
    async def update(
        self, sid: str, user_id: str, employee_id: str, data: dict
    ) -> bool:
    ...
    async def delete(self, sid: str, user_id: str, employee_id: str) -> None:
        await self.collection.delete_one(
            {"id": sid, "user_id": user_id, "employee_id": employee_id}
        )
