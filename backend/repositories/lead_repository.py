...
class LeadRepository:
    ...
    async def update_status(self, lid: str, user_id: str, status: str, now_iso_str: str) -> None:
        ...
        await self.collection.update_one(
            ...
            {"$set": {"status": status, "updated_at": now_iso_str, "last_interaction_at": now_iso_str}},
        )

    async def log_contact(self, lid: str, user_id: str, now_iso_str: str, notes: Optional[str] = None) -> bool:
        ...
        if notes is not None:
            data["notes"] = notes
        result = await self.collection.update_one({"id": lid, "user_id": user_id}, {"$set": data})
