                ...
                {dayAppts.map((a) => {
                  ...
                  return (
                    <div key={a.id} data-testid={`appt-${a.id}`} className="bg-[#F9F9F8] border-l-2 border-[#FF5A00] p-2 rounded-r-md">
                      ...
                      <button onClick={async () => { await api.delete(`/appointments/${a.id}`); load(); }} className="text-[10px] text-[#A1A1AA] mt-1">elimina</button>
                    </div>
                  );
                })}
              </div>
...
function ApptForm({ clients, onSave }) {
  ...
  return (
    <form onSubmit={async (e) => { e.preventDefault(); const start = new Date(f.start).toISOString(); await onSave({ ...f, start }); }} className="space-y-3">
      <div>
      ...
      <button data-testid="save-appt-button" type="submit" className="w-full bg-[#0A192F] text-white py-2.5 rounded-md text-[13px] font-medium">Salva appuntamento</button>
    </form>
  );
}
