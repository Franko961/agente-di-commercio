      ...
      <div className="bg-white border border-[#E4E4E1] rounded-md p-3 space-y-2.5">
        {steps.map(([label, done]) => (
          <div key={label} className="flex items-center gap-2">
            ...
            <span className={`text-[11px] ${done ? "text-[#A1A1AA] line-through" : "text-[#0A0A0A] font-medium"}`}>{label}</span>
          </div>
        ))}
      </div>
    </MockupFrame>
        ...
        {orders.map(([num, client, total, status, color]) => (
          <div key={num} className="bg-white border border-[#E4E4E1] rounded-md p-2.5">
            <div className="flex items-center justify-between mb-1">
              <span className="font-mono text-[8px] text-[#A1A1AA]">{num}</span>
  ...
  return (
    <MockupFrame>
      <div className="bg-white border border-[#E4E4E1] rounded-md p-2.5 mb-2.5 text-center">
        <div className="font-mono text-[8px] uppercase tracking-widest text-[#A1A1AA]">Provvigioni maturate</div>
        ...
        <div className="space-y-1">
          {tiers.map((t) => (
            <div key={t.threshold} className="flex items-center justify-between">
              <span className={`text-[9px] ${t.reached ? "text-[#0A0A0A] font-medium" : "text-[#A1A1AA]"}`}>{t.threshold}</span>
              <span className={`text-[9px] font-mono ${t.reached ? "text-[#059669]" : "text-[#A1A1AA]"}`}>{t.reached ? "✓ " : ""}{t.bonus}</span>
            </div>
          ))}
        </div>
      </div>
  ...
  return (
    <MockupFrame>
      <div className="space-y-2.5">
        ...
        <div className="flex items-center gap-1.5 text-[8px] text-[#A1A1AA] justify-center pt-1">
